...
class QLora:
    def __init__(
        ...
        lambda_losses: Union[Sequence[float], None] = None,
...
class ReasonCodeSFTTrainer(SFTTrainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Loss weights [decision, reason, structural]. None -> the class default
        # LAMBDA_LOSSES, so behaviour is unchanged unless a config overrides it.
        # Set from YAML to run the joint-vs-decision-only ablation: the joint arm is
        # the default [0.2, 1.0, 0.01]; the decision-only arm zeroes the reason term.
        # NB zeroing reason also lowers total gradient magnitude — see the renormalised
        # control arm in the ablation plan before attributing a delta to reason signal.
        if lambda_losses is not None:
            lam = [float(x) for x in lambda_losses]
            if len(lam) != 3:
                raise ValueError(
                    f"lambda_losses must be [decision, reason, structural]; got {lam!r}"
                )
            self.LAMBDA_LOSSES = lam  # instance attr shadows the class constant
        print(f"[loss] LAMBDA_LOSSES (decision, reason, struct) = {self.LAMBDA_LOSSES}")
        # 'maybe' (uncertainty) decision token controls
        self.maybe_weight = (
            float(maybe_weight)
            if maybe_weight is not None
            else float(self.MAYBE_WEIGHT)
        )
